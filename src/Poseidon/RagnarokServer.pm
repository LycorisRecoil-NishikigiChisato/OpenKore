	...
	if ($switch eq '01DB' || $switch eq '0204') { # Secure Login
		...
		$data = pack("v", 0x0AC9) . # header
			...
			$data = pack("v", 0x0AC4) . # header
				...
				$serverUsers .
				pack("x130");
		} elsif($switch eq '0A76' || $self->{type}->{$config{server_type}}->{account_server_info} eq '0276') { # tRO
...
sub SendCharacterList
{
	...
	my $data;
	if ($self->{type}->{$config{server_type}}->{received_characters} eq '082D') {
		...
		$data = pack("v", 0x099D) .
		pack("v", $len + 4);
	} elsif ($self->{type}->{$config{server_type}}->{received_characters} eq '0B72') {
		$data = pack("v", 0x0B72) .
		pack("v", $len + 4);
	} else {
...
sub SendMapLogin {
	...
	$client->send(pack("v C v a4", 0x0009, 0x00, 0x0283, $accountID));

	if ( $config{server_type} =~ /^twRO/ ) { # kRO
		$client->send(pack("v C v", 0x0009, 0x00, 0x0ADE) . pack("V", 0x00));
	}
	...
	if ($self->{type}->{$config{server_type}}->{map_loaded} eq '0A18') {
		...
		$client->send(pack("v C v", 0x0010, 0x00, 0x02EB) . pack("V", getTickCount) . getCoordString($posX, $posY, 1) . pack("C*", 0x00, 0x00) .  pack("C*", 0x00, 0x00));
	} else {
	...
	$client->send(pack("v C v2", 0x0007, 0x00, 0x013A, 1));
	...
	$client->send(pack("v C v2 C12 v14", 0x002F, 0x00, 0x00BD, 100, 99, 11, 99, 11, 99, 11, 99, 11, 99, 11, 99, 11, 999, 999, 999, 999, 999, 999, 999, 999, 999, 999, 999, 100, 190, 3));
